...
class Agent:
    ...
    def build_prompt(self, user_input: str, session_id: str) -> List[Message]:
        ...
        history = self.memory.get_history(limit=20, session_id=session_id)
    ...
    async def chat_stream(self, user_input: str, session_id: str) -> AsyncGenerator[ChatStreamChunk, None]:
        ...
        # Persist after building the prompt so the current message is included once.
        self.memory.add_turn("user", user_input, session_id=session_id)
